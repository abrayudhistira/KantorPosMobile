...
class _UangMasukScreenState extends State<UangMasukScreen> {
  final UangMasukService _service = UangMasukService();
  List<UangMasuk> _uangMasukList = [];
  ...
  void _showForm({UangMasuk? uangMasuk}) {
    bool isEdit = uangMasuk != null;
    
    if (isEdit) {
      _nominalController.text = uangMasuk.nominal;
      _keteranganController.text = uangMasuk.keterangan;
      _selectedDate = uangMasuk.tanggal;
      _selectedUangMasuk = uangMasuk;
    } else {
      _nominalController.clear();
      _keteranganController.clear();
      _selectedDate = DateTime.now();
      _selectedUangMasuk = null;
    }
  ...
  Future<void> _handleSubmit() async {
    ...
    final uangMasuk = UangMasuk(
      id: _selectedUangMasuk?.id ?? 0,
    ...
    try {
      if (_selectedUangMasuk != null) {
        await _service.updateUangMasuk(uangMasuk);
      } else {
        await _service.createUangMasuk(uangMasuk);
      }
      _fetchData();
  ...
  //   final total = _uangMasukList.fold(
  //     0.0, (sum, item) => sum + double.parse(item.nominal));
  ...
  Widget _buildTransactionList() {
    return ListView.builder(
      padding: const EdgeInsets.all(16),
      itemCount: _uangMasukList.length,
      itemBuilder: (context, index) {
        final transaction = _uangMasukList[index];
        return Card(
          ...
          child: ListTile(
            ...
            trailing: PopupMenuButton(
              ...
              onSelected: (value) {
                if (value == 'edit') {
                  _showForm(uangMasuk: transaction);
      ...
      builder: (context) => AlertDialog(
        ...
        actions: [
          ...
          TextButton(
            onPressed: () async {
              try {
                await _service.deleteUangMasuk(id);
                _fetchData();
                Navigator.pop(context);
